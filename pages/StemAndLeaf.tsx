...
const processStemData = (
    ...
    for (let i = startStemIdx; i <= endStemIdx; i++) {
        let label = "";
        if (splitFactor === 1) label = i.toString();
        else {
            const baseStem = Math.floor(i / splitFactor);
            label = baseStem.toString();
        }
        rows.push({ stemLabel: label, stemValue: i, leavesLeft: [], leavesRight: [] });
    }
  ...
  const renderPlotContent = () => {
    ...
    rows.forEach((row, rIdx) => {
        ...
        row.leavesRight.forEach((val, i) => {
            ...
            els.push(<text key={`r-${rIdx}-${i}`} x={x} y={y} fontSize={fs} fill={config.colorRight} fontFamily="Times New Roman" textAnchor="middle">{txt}</text>);
        });
    });
      ...
      <div className="flex flex-1 overflow-hidden">
                    ...
                    <>
                        <div className="space-y-3">
                            <div>
                                ...
                                <textarea value={data1} onChange={(e) => setData1(e.target.value)} className="w-full h-24 border border-gray-300 rounded p-2 text-xs font-mono" placeholder="e.g. 12, 15, 22..." />
                                <div className="mt-1 flex gap-2">
                                    <input type="text" placeholder="Label" value={config.labelLeft} onChange={(e) => setConfig({...config, labelLeft: e.target.value})} className="flex-1 border rounded px-2 py-1 text-xs" />
                            ...
                            <div>
                                ...
                                <textarea value={data2} onChange={(e) => setData2(e.target.value)} className="w-full h-24 border border-gray-300 rounded p-2 text-xs font-mono" placeholder="e.g. 12, 15, 22..." />
                                <div className="mt-1 flex gap-2">
                                    <input type="text" placeholder="Label" value={config.labelRight} onChange={(e) => setConfig({...config, labelRight: e.target.value})} className="flex-1 border rounded px-2 py-1 text-xs" />
                        ...
                        <div className="pt-4 border-t border-gray-100 space-y-3">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select value={config.splitFactor} onChange={(e) => setConfig({...config, splitFactor: parseInt(e.target.value)})} className="w-full border border-gray-300 rounded p-1 text-xs">
                            ...
                            <div className="flex flex-col gap-2 pt-2">
                                <label className="flex items-center gap-2 text-xs text-gray-600 cursor-pointer">
                                    <input type="checkbox" checked={config.autoSort} onChange={(e) => setConfig({...config, autoSort: e.target.checked})} className="rounded text-indigo-600"/><ArrowDownAZ size={14} className="text-gray-400"/> Auto-Sort Leaves
                                </label>
                                <label className="flex items-center gap-2 text-xs text-gray-600 cursor-pointer">
                                    <input type="checkbox" checked={config.showQuartiles} onChange={(e) => setConfig({...config, showQuartiles: e.target.checked})} className="rounded text-indigo-600"/><CircleDot size={14} className="text-gray-400"/> Show Visual Quartiles
                                </label>
                                <div className="flex gap-4 mt-2">
                                    <label className="flex items-center gap-2 text-xs text-gray-600 cursor-pointer"><input type="checkbox" checked={config.showKey} onChange={(e) => setConfig({...config, showKey: e.target.checked})} className="rounded text-indigo-600"/>Show Key</label>
                ...
                {activeTab === 'style' && (
                    <div className="space-y-4">
                        ...
                        {config.showQuartiles && (
                            <div className="pt-4 border-t border-gray-100 space-y-3">
                                <h3 className="text-xs font-bold text-gray-500 uppercase flex items-center gap-2"><CircleDot size={12}/> Quartile Styling</h3>
                                <div className="grid grid-cols-2 gap-3">
                                    <div><label className="block text-xs text-gray-500 mb-1">Exact Color</label><input type="color" value={config.quartileColorExact} onChange={(e) => setConfig({...config, quartileColorExact: e.target.value})} className="w-full h-6 border rounded cursor-pointer" /></div>
                                    <div><label className="block text-xs text-gray-500 mb-1">Split Color</label><input type="color" value={config.quartileColorSplit} onChange={(e) => setConfig({...config, quartileColorSplit: e.target.value})} className="w-full h-6 border rounded cursor-pointer" /></div>
                                </div>
                                <div className="grid grid-cols-2 gap-3">
                                    <div><label className="block text-xs text-gray-500 mb-1">Offset X</label><input type="number" value={config.quartileOffsetX} onChange={(e) => setConfig({...config, quartileOffsetX: parseFloat(e.target.value) || 0})} className="w-full border rounded p-1 text-xs" /></div>
                                    <div><label className="block text-xs text-gray-500 mb-1">Offset Y</label><input type="number" value={config.quartileOffsetY} onChange={(e) => setConfig({...config, quartileOffsetY: parseFloat(e.target.value) || 0})} className="w-full border rounded p-1 text-xs" /></div>
                                </div>
                            </div>
                        )}
                        <div className="pt-4 border-t border-gray-100 space-y-3">
                            <label className="block text-xs font-bold text-gray-500 uppercase flex items-center gap-2"><LayoutTemplate size={12}/> Key Position Preset</label>
                         ...
                         <div className="pt-4 border-t border-gray-100">
                            ...
                            <div className="space-y-2">
                                <button onClick={handleFitCanvas} className="w-full py-1.5 flex items-center justify-center gap-2 bg-indigo-50 text-indigo-700 border border-indigo-100 rounded text-xs font-medium hover:bg-indigo-100 transition-colors"><Maximize size={12} /> Fit Canvas to Content</button>
                                <label className="flex items-center gap-2 text-xs text-gray-500 cursor-pointer"><input type="checkbox" checked={showDebug} onChange={(e) => setShowDebug(e.target.checked)} className="rounded border-gray-300"/><Scan size={12} /> Show Debug Bounds</label>
                            </div>
                        </div>
                    </div>
                )}
        ...
        <main className="flex-1 bg-gray-100 overflow-hidden flex flex-col">
          <div ref={containerRef} className="flex-1 overflow-auto flex items-center justify-center p-8 bg-neutral-100 cursor-crosshair">
            <div className={`bg-white shadow-2xl transition-all duration-200 ease-in-out relative ${cropMode ? 'cursor-crosshair' : 'cursor-default'}`} style={{ transform: `scale(${previewScale})`, transformOrigin: 'top center', opacity: hasInitialCrop ? 1 : 0 }} onMouseDown={handleCropMouseDown}>
              <svg id="stem-svg" width={widthPixels} height={heightPixels} viewBox={customViewBox || `0 0 ${widthPixels} ${heightPixels}`} xmlns="http://www.w3.org/2000/svg" style={{ display: 'block', fontFamily: 'Times New Roman' }}>
                <rect x="0" y="0" width={widthPixels} height={heightPixels} fill="white" />
                <g className="data-layer">{renderPlotContent()}</g>
                {config.showKey && (
                ...
                {cropMode && selectionBox && (<rect x={selectionBox.x} y={selectionBox.y} width={selectionBox.w} height={selectionBox.h} fill="rgba(59, 130, 246, 0.2)" stroke="#2563eb" strokeWidth={2 / previewScale} strokeDasharray={`${5/previewScale},${5/previewScale}`} pointerEvents="none" />)}
              </svg>
            </div>
          </div>
           {cropMode && (<div className="absolute top-4 left-1/2 -translate-x-1/2 bg-blue-600 text-white px-4 py-2 rounded-full shadow-lg text-sm font-bold animate-pulse pointer-events-none">Drag to Crop Graph</div>)}
        </main>
      </div>
    </div>
  );
